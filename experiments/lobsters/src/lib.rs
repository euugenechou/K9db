...
pub struct Timeline {
  ...
  // the first histogram is 0-1s after start, the second 1-2s after start, then
  // 2-4s, etc.
...
impl Default for Histograms {
  fn default() -> Self {
    Self { processing: Histogram::new_with_bounds(1, 60_000_000, 3).unwrap(),
...
impl Timeline {
  ...
  pub fn write<W: std::io::Write,
                 S: hdrhistogram::serialization::Serializer>(
    &self,
    ...
    for (i, hs) in self.histograms.iter().enumerate() {
      ...
      if self.total_duration != Duration::new(0, 0)
         && start + dur > self.total_duration
      {
